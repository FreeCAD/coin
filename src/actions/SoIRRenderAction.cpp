...
public:
  explicit SoRetainedMutationTransaction(SoIRRenderAction & action)
    : action(action),
      commandCount(action.drawlist.getNumCommands()),
      resourceCount(action.drawlist.getNumGeometryResources()),
  ...
  int originalCommandCount() const { return this->commandCount; }
  ...
  void finish(bool keepAllocations)
  {
    ...
    this->action.drawlist.truncate(this->commandCount);
    ...
    this->action.drawlist.truncateGeometryResources(this->resourceCount);
    for (auto it = PRIVATE(&this->action)->geometrySources.begin();
         it != PRIVATE(&this->action)->geometrySources.end();) {
      if (it->second > static_cast<SoGeometryHandle>(this->resourceCount)) {
