...
SoIRRenderAction::applyRenderStage(SoRenderCommand & command)
{
  if (this->isAfterMainStage()) {
    ...
    // A foreground traversal is an overlay pass even when the child node's
    // own depth state would otherwise classify it as opaque or transparent.
...
SoIRRenderStageScope::SoIRRenderStageScope(SoIRRenderAction & action,
                                           SoRenderStage stage)
  : action(&action), active(stage != SoRenderStage::Main),
    previousStage(action.getRenderStage())
{
