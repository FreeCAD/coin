...
textureFiltersFromQuality(const float quality, SoTextureData & texture)
{
  ...
  static const float anisotropicLimit =
    textureQualityLimit("COIN_TEX2_ANISOTROPIC_LIMIT", 0.85f);
  ...
  // executor will clamp this request to the active driver's capability and
  // use ordinary filtering when anisotropy is unavailable.
  texture.maxAnisotropy = quality > anisotropicLimit ? 2.0f : 1.0f;
}
