...
main()
{
  ...
  if (point.state.raster.pointSize != 6.0f) {
    std::cerr << "FAIL: retained point raster state lost its semantic size" << std::endl;
    result = 1;
  }
  ...
  for (int i = 0; i < action.getDrawList().getNumCommands(); ++i) {
    ...
    if (command.pixelRaster.kind != SO_PIXEL_RASTER_TEXT) continue;
  ...
  for (int i = 0; i < action.getDrawList().getNumCommands(); ++i) {
    const SoRenderCommand & command = action.getDrawList().getCommand(i);
    if (command.state.raster.fillMode == 1 &&
