...
main()
{
  ...
  SoSeparator * subtree = new SoSeparator;
  subtree->ref();
  subtree->addChild(new SoCube);
  action.traverseAdditionalSubtree(subtree, context);
  if (action.getDrawList().getNumCommands() != 3 ||
      action.getDrawList().getCommand(2).modelMatrix != model) {
    std::cerr << "FAIL: deferred subtree traversal did not use the copied model context"
              << std::endl;
    result = 1;
  }
  subtree->unref();
  ...
  if (action.getDrawList().getNumCommands() != 4 ||
      action.getDrawList().getCommand(3).viewMatrix == viewing) {
