...
struct Measurement {
  ...
  double incrementalUpdateMedianMs = 0.0;
                          ...
                          bool hoverOnly = false)
{
  ...
  if (workload == WorkloadKind::SharedAssemblyRecipe) {
    ...
    for (int sample = 0; sample < samples; ++sample) {
      ...
      SoRenderManager::PickRequest request;
      const Clock::time_point completionStart = Clock::now();
      start = Clock::now();
      if (!manager.requestPickClosestAsync(
            refreshCursor[0], refreshCursor[1], 4, request)) {
        unavailable = "manager asynchronous hover request failed";
        manager.releaseRenderBackendResources();
        manager.setCamera(NULL);
        manager.setSceneGraph(NULL);
        camera->unref();
      ...
      asyncRequestTimes.push_back(elapsedMs(start));

      SoPickedPoint * picked = NULL;
      start = Clock::now();
      SoRenderManager::PickStatus status =
        manager.pollPickClosestAsync(request, picked);
      asyncPollTimes.push_back(elapsedMs(start));
      if (status == SoRenderManager::PickStatus::PENDING) {
        glFinish();
        status = manager.pollPickClosestAsync(request, picked);
      }
      asyncCompletionTimes.push_back(elapsedMs(completionStart));
      if (status != SoRenderManager::PickStatus::HIT || !picked) {
        delete picked;
        unavailable = "manager asynchronous hover did not resolve a hit";
        manager.releaseRenderBackendResources();
        manager.setCamera(NULL);
        manager.setSceneGraph(NULL);
        camera->unref();
    ...
    asyncHover.workload = "shared_assembly_async_hover_" +
      std::to_string(occurrenceCount);
    ...
    asyncHover.asyncPickRequestP95Ms =
      percentile(asyncRequestTimes, 0.95);
    ...
    asyncHover.pixelChecksum = checksumPixels(context.readPixels());
  ...
  std::vector<Measurement> & results, std::string & unavailable)
{
  ...
  for (int sample = 0; sample < samples; ++sample) {
    ...
    drawlist.getCommandPreservingPickTopology(0).modelMatrix = matrix;
    if (!backend.updatePickBuffer(drawlist, plan, params)) {
  ...
  const auto measureSelection = [&](const char * label,
    ...
    for (int sample = 0; sample < samples; ++sample) {
      ...
      const uint64_t expectedTargets =
  ...
  const bool valid =
    measureSelection("shared_assembly_selection_1_percent", onePercent, false) &&
    measureSelection("shared_assembly_selection_10_percent", tenPercent, false) &&
    measureSelection("shared_assembly_selection_churn", tenPercent, true) &&
    measureSelection(
      "shared_assembly_subelement_selection", tenPercent, true, true) &&
    measureSelection("shared_assembly_preselection", 1, true);
                           ...
                           std::string & unavailable)
{
  ...
  for (int occurrence = 0; occurrence < occurrenceCount; ++occurrence) {
    mutations.transforms[static_cast<size_t>(occurrence)]->translation =
      SbVec3f(0.0f, 0.0f, -0.05f * static_cast<float>(occurrence));
  }
  ...
  SoPickedPoint * warmHit = NULL;
  if (!manager.pickClosest(128, 128, 0, warmHit) || !warmHit) {
    unavailable = "depth-stack scene produced no center hit";
    delete warmHit;
    manager.releaseRenderBackendResources();
    manager.setCamera(NULL);
    manager.setSceneGraph(NULL);
  ...
  for (int requested : requestedLayers) {
    ...
    for (int sample = 0; sample < curveSamples; ++sample) {
      ...
      const Clock::time_point start = Clock::now();
      if (!manager.pickDepthStack(128, 128, 0, maxLayers, stack,
                                  maxLayers)) {
                   ...
                   const Options & options)
{
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    ...
    out << "    {\"workload\": \"" << r.workload
        ...
        << ", \"incremental_update_median_ms\": "
        << r.incrementalUpdateMedianMs
